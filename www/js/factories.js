angular.module('GenesisApp.factories', [])
...
.factory('UserDetails', function (API_URL, NetworkService, $q){
  return {
	  getDetails: function(){
		  var params = {
		  ...
		  options.success = function(data){
			  ...
			  localStorage.setItem('userDetails', data.entry_list[0]);
		  };
...
.factory('PushLinkContact', function ($ionicPopup, NetworkService, API_URL){
  return {
	  setRelationship: function(){
		  var regisParams = {
				  ...
				  "link_field_name": "contacts_gi_mobile_registrations_1",
				  "related_ids": [localStorage.getItem('registrationId')],
				  "delete": 0
		  };
		  var dataToPost = {
		  ...
		  options.data = dataToPost;
		  options.success = function(data){
			  console.log('Push link contact Success!!');
			  console.log(data);
		  };
		  options.error = function(error){
			  $ionicPopup.alert({
				  title: 'Error',
				  template: 'There is some problem in linking contact for notifications.'
			  });
		  };
...
.factory('GetAppNotifications', function ($ionicPopup, NetworkService, API_URL){
  return {
